...
describe 'タスクモデル機能', type: :model do
  ...
  context 'タスクのタイトルと詳細に内容が記載されている場合' do
    it 'バリデーションが通る' do
      task = Task.new(title: '失敗タスク', content: '失敗タスク')
      expect(task).to be_valid
    end
  end

  describe '検索機能' do
    let!(:task) { FactoryBot.create(:task, title: 'タイトル1', content: 'コンテント1', status: '未着手')}
    let!(:second_task) { FactoryBot.create(:second_task, title: "タイトル2", content: 'コンテンt2', status: '着手中')}
    let!(:third_task) { FactoryBot.create(:third_task, title: "タイトル3", content: 'コンテント3', status: '完了')}
